...
export interface CloudFrontS3WebSiteConstructProps extends cdk.StackProps {
    ...
    /**
     * The name of the video assets bucket for video processing
     */
    readonly videoAssetsBucketName?: string;

    /**
     * The video assets bucket object (if provided)
     */
    readonly videoAssetsBucket?: cdk.aws_s3.IBucket;
...
export class CloudFrontS3WebSiteConstruct extends Construct {
    ...
    constructor(parent: Construct, name: string, props: CloudFrontS3WebSiteConstructProps) {
        ...
        const s3origin = cdk.aws_cloudfront_origins.S3BucketOrigin.withOriginAccessControl(
            this.siteBucket,
        );
        ...
        // Add video assets bucket as additional origin if provided
        if (props.videoAssetsBucket) {
            const videoAssetsOrigin = cdk.aws_cloudfront_origins.S3BucketOrigin.withOriginAccessControl(
                props.videoAssetsBucket,
            );

            cloudFrontDistribution.addBehavior("/harvested-clips/*", videoAssetsOrigin, {
                cachePolicy: new cdk.aws_cloudfront.CachePolicy(this, "VideoAssetsCachePolicy", {
            ...
            // Add behavior for generated reels
            cloudFrontDistribution.addBehavior("/reels/*", videoAssetsOrigin, {
                cachePolicy: new cdk.aws_cloudfront.CachePolicy(this, "ReelsCachePolicy", {
                    defaultTtl: cdk.Duration.hours(24),
                    maxTtl: cdk.Duration.days(365),
                    minTtl: cdk.Duration.seconds(0),
                }),
        ...
        // Configure Storage using Amplify v6 format
        const storageBucket = props.videoAssetsBucketName;
        if (storageBucket) {
        ...
        // Add video assets bucket configuration for custom access
        if (props.videoAssetsBucketName) {
            amplifyConfig.aws_video_assets_bucket = props.videoAssetsBucketName;
            amplifyConfig.aws_video_assets_bucket_region = region;
        }

        // Add CloudFront domain URL for video asset access
        amplifyConfig.aws_cloudfront_url = `https://${cloudFrontDistribution.distributionDomainName}`;

        const appDeployment = new cdk.aws_s3_deployment.BucketDeployment(
            this,
        ...
        appDeployment.node.addDependency(this.siteBucket);

        // export any cf outputs
        new cdk.CfnOutput(this, "SiteBucket", { value: this.siteBucket.bucketName });
